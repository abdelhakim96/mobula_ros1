...
file_name_mhe_with_horizon = os.path.join(folder_name, "mhe_with_horizon.txt")  # New file
...
ref_positions_x_mhe = []  # New list
ref_positions_y_mhe = []  # New list
actual_positions_x_mhe = []  # New list
actual_positions_y_mhe = []  # New list
...
# Read data from the fourth file (mhe_with_horizon)
with open(file_name_mhe_with_horizon, 'r') as file_mhe_with_horizon:
    for line in file_mhe_with_horizon:
        data = line.strip().split(',')
        ref_positions_x_mhe.append(float(data[0]))
        ref_positions_y_mhe.append(float(data[1]))
        actual_positions_x_mhe.append(float(data[3]))
        actual_positions_y_mhe.append(float(data[4]))
...
# Plot for MHE with horizon
plt.plot(actual_positions_x_mhe[:subset_size], actual_positions_y_mhe[:subset_size], label='MHE with Horizon', color='purple', linestyle='--')
...
error_mhe = np.sqrt((np.array(actual_positions_x_mhe[:1000]) - np.array(ref_positions_x_mhe[:1000]))**2 +
                    (np.array(actual_positions_y_mhe[:1000]) - np.array(ref_positions_y_mhe[:1000]))**2)
...
plt.plot(range(len(error_mhe)), error_mhe, label='MHE with Horizon', color='purple')
